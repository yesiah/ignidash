...
export default function SectionSelector({ activeSection, setActiveSection }: SectionSelectorProps) {
  ...
  return (
    <>
      <div className="border-border/50 from-emphasized-background to-background fixed top-[4.0625rem] z-30 -mx-2 w-full border-b border-dashed bg-gradient-to-bl py-2 sm:-mx-3 lg:top-0 lg:-mx-4 lg:w-[calc(100%-18rem)] lg:py-4 lg:group-data-[state=collapsed]/sidebar:w-[calc(100%-4rem)]">
        <div className="mr-4 flex items-stretch justify-between sm:mr-6 lg:mr-8">
          <nav
            aria-label="Tabs"
            className="divide-border/50 border-border/50 -my-2 flex divide-x divide-dashed border-r border-dashed lg:-my-4"
          >
            {tabs.map((tab) => (
              <button
                key={tab.name}
