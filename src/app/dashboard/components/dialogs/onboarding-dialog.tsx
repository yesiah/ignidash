    ...
    (newDisclosure: DisclosureState) => {
      if (activeDisclosure?.open && activeDisclosure.key !== newDisclosure.key) {
        ...
        switch (newDisclosure.key) {
          case 'dashboard':
            targetRef = dashboardButtonRef.current;
            break;
          case 'simulator':
            targetRef = simulatorButtonRef.current;
            break;
          case 'insights':
            targetRef = insightsButtonRef.current;
            break;
          case 'links':
            targetRef = linksButtonRef.current;
            break;
        }
          ...
          {({ open, close }) => (
            <>
              <DisclosureButton
                ...
                onClick={() => {
                  if (!open) close();
                  toggleDisclosure({ open, close, key: 'dashboard' });
                }}
                onKeyDown={(event) => {
              ...
              <DisclosurePanel className="space-y-2 pt-4">
                ...
                <Text>
                  From here, you can view existing Simulator plans, create new ones, and track your net worth by adding assets and
                  liabilities.
                </Text>
                <Text>
          ...
          {({ open, close }) => (
            <>
              <DisclosureButton
                ...
                onClick={() => {
                  if (!open) close();
                  toggleDisclosure({ open, close, key: 'simulator' });
                }}
                onKeyDown={(event) => {
                  if (event.key === 'Enter' || event.key === ' ') {
                    if (!open) close();
              ...
              <DisclosurePanel className="space-y-2 pt-4">
                ...
                <Text>
                  ...
                  <SettingsIcon className="text-foreground mx-1 inline size-5 shrink-0" aria-label="Simulation Settings button icon" /> next
                  to <Strong>Results</Strong> and selecting one of the Monte Carlo modes. Drill down into full individual simulations by
                  clicking on rows in the table at the bottom of the page.
                </Text>
              </DisclosurePanel>
            </>
          )}
          ...
          {({ open, close }) => (
            <>
              <DisclosureButton
                ...
                onClick={() => {
                  if (!open) close();
                  toggleDisclosure({ open, close, key: 'insights' });
                }}
                onKeyDown={(event) => {
                  if (event.key === 'Enter' || event.key === ' ') {
                    if (!open) close();
          ...
          {({ open, close }) => (
            <>
              <DisclosureButton
                ...
                onClick={() => {
                  if (!open) close();
                  toggleDisclosure({ open, close, key: 'links' });
                }}
                onKeyDown={(event) => {
                  if (event.key === 'Enter' || event.key === ' ') {
                    if (!open) close();
