...
export const singleSimulationReturnsTableRowSchema = z.object({
  ...
  age: z.number(),
  phaseName: z.string().nullable(),
  totalAnnualGain: z.number().nullable(),
  totalCumulativeGain: z.number().nullable(),
...
const SINGLE_SIMULATION_RETURNS_COLUMNS = {
  ...
  age: { title: 'Age', format: 'number' },
  phaseName: { title: 'Phase Name', format: 'string' },
  totalAnnualGain: { title: 'Total Annual Gain', format: 'currency' },
  totalCumulativeGain: { title: 'Total Cumulative Gain', format: 'currency' },
